...
function workloadContent(hostId: number | undefined, workloads: ReturnType<typeof useDockerWorkloads>, adoptPending: boolean, onAdopt: (id: string) => void, onRemove: (item: { id: string; name: string }) => void) {
  ...
  return <div className={ui.tableWrap}><table><thead><tr><th>Name</th><th>Image</th><th>Ports</th><th>Status</th><th>Discovery</th><th>Verwaltung</th><th /></tr></thead><tbody>{workloads.data.map((item) => <tr key={item.id}><td><strong>{item.name}</strong><br /><small className={ui.muted}>{item.id.slice(0, 12)}</small></td><td>{item.image}</td><td>{item.ports.length ? item.ports.join(", ") : "—"}</td><td>{item.status}<br /><small className={ui.muted}>{item.presence === "missing" ? "Nicht mehr gefunden" : "Aktuell gefunden"}</small></td><td><time dateTime={item.discovered_at}>{new Date(item.discovered_at).toLocaleString()}</time></td><td><span className={cn(ui.statusBadge, managementClass(item.management_state) === "success" ? ui.statusSuccess : item.presence === "missing" ? ui.statusNeutral : ui.statusPending)}>{item.presence === "missing" ? "nicht mehr vorhanden" : managementLabel(item.management_state)}</span></td><td><div className={ui.actionRow}>{item.management_state === "managed" || item.presence === "missing" ? null : <button className={ui.button} type="button" disabled={adoptPending} onClick={() => onAdopt(item.id)}>Aufnehmen</button>}<button className={ui.dangerButton} type="button" onClick={() => onRemove(item)}>Entfernen</button></div></td></tr>)}</tbody></table></div>;
}
