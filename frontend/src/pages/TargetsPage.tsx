      ...
      <section className="panel workflow-panel">
        ...
        <form onSubmit={(event) => { event.preventDefault(); create.mutate(); }}>
          <div className="workflow-grid">
            ...
            <label><span className="field-label">Deployment-Secret <span className="field-info" title="Zugangsdaten für die Verbindung zum Ziel, zum Beispiel ein SSH-Passwort oder ein SSH-Private-Key." aria-label="Info zum Deployment-Secret">i</span></span><div className="inline-field"><select value={credentialSecret} onChange={(event) => setCredentialSecret(event.target.value)} required><option value="">Secret auswählen</option>{activeSecrets.map((item) => <option key={item.metadata.metadata.id} value={item.metadata.metadata.id}>{item.metadata.metadata.name}</option>)}</select><button className="secondary-button" type="button" onClick={() => { setNewSecretFor("credential"); setNewSecretKind(selectedKind.transport === "ssh" ? "ssh_password" : "generic"); }}>＋ Neu</button></div></label>
            <label><span className="field-label">Agent-Token <span className="field-info" title="Geheimer Token, mit dem sich der installierte lxcup-Agent beim Controller authentifiziert. Er ist nicht das SSH-Passwort." aria-label="Info zum Agent-Token">i</span></span><div className="inline-field"><select value={agentSecret} onChange={(event) => setAgentSecret(event.target.value)} required><option value="">Secret auswählen</option>{activeSecrets.map((item) => <option key={item.metadata.metadata.id} value={item.metadata.metadata.id}>{item.metadata.metadata.name}</option>)}</select><button className="secondary-button" type="button" onClick={() => { setNewSecretFor("agent"); setNewSecretKind("agent_token"); }}>＋ Neu</button></div></label>
