...
export function EnrollmentPage() {
  ...
  return <>
    <header className="page-header"><div><p className="eyebrow">LXC-Container</p><h1>LXC aufnehmen</h1><p className="muted">Verknüpfe einen entdeckten LXC mit seinem Zugangsprofil und rolle den Agenten kontrolliert aus.</p></div><Link className="secondary-button" to="/targets/lxc">LXC-Zugang anlegen</Link></header>
    <section className="panel enrollment-panel">
      {enrollmentId ? <EnrollmentProgress enrollment={enrollment} selectedContainer={selectedContainer} state={state} startOnboarding={startOnboarding} health={health} healthJobId={healthJobId} onRetry={retry} /> : <EnrollmentStartForm available={available} targets={targets.data ?? []} selectedContainer={selectedContainer} selectedTarget={selectedTarget} startOnboarding={startOnboarding} submitError={submitError} containersLoading={containers.isLoading} onContainerChange={setSelectedContainer} onTargetChange={setSelectedTarget} onStartOnboardingChange={setStartOnboarding} onStart={startEnrollment} />}
    </section>
  </>;
}
...
function EnrollmentStartForm({ available, targets, selectedContainer, selectedTarget, startOnboarding, submitError, containersLoading, onContainerChange, onTargetChange, onStartOnboardingChange, onStart }: Readonly<{ available: Array<{ id: number; name: string }>; targets: Array<{ id: string; name: string; address: string; state: string; kind: string }>; selectedContainer: string; selectedTarget: string; startOnboarding: boolean; submitError?: string; containersLoading: boolean; onContainerChange: (value: string) => void; onTargetChange: (value: string) => void; onStartOnboardingChange: (value: boolean) => void; onStart: () => Promise<void> }>) {
  ...
  return <><label className="workflow-field" htmlFor="enrollment-container"><span>1. LXC aus dem Inventar auswählen</span><select id="enrollment-container" value={selectedContainer} onChange={(event) => onContainerChange(event.target.value)}><option value="">Bitte auswählen…</option>{available.map((container) => <option key={container.id} value={container.id}>{container.name} · VMID {container.id}</option>)}</select></label>{startOnboarding && <label className="workflow-field" htmlFor="enrollment-target"><span>2. LXC-Zugangsprofil auswählen</span><select id="enrollment-target" value={selectedTarget} onChange={(event) => onTargetChange(event.target.value)}><option value="">Bitte Ziel auswählen…</option>{eligibleTargets.map((target) => <option key={target.id} value={target.id}>{target.name} · {target.address}</option>)}</select><span className="muted">Das Zugangsprofil liefert SSH-Zugang, Host-Key und Agent-Token für diesen LXC.</span></label>}<label className="confirm-field"><input type="checkbox" checked={startOnboarding} onChange={(event) => onStartOnboardingChange(event.target.checked)} /><span>3. Agent installieren und nach erfolgreicher Verbindung einen Healthcheck ausführen.</span></label>{!containersLoading && available.length === 0 && <p className="muted">Keine laufenden, noch auswählbaren LXCs entdeckt.</p>}{startOnboarding && eligibleTargets.length === 0 ? <p className="muted">Noch kein LXC-Zugangsprofil vorhanden. <Link className="text-link" to="/targets/lxc">Jetzt anlegen →</Link></p> : null}{submitError && <p className="error-state" role="alert">{submitError}</p>}<button className="primary-button" type="button" disabled={!selectedContainer || (startOnboarding && !selectedTarget)} onClick={() => void onStart()}>LXC aufnehmen</button></>;
}

type HealthMutationView = Readonly<{ isPending: boolean; error: unknown }>;

function EnrollmentProgress({ enrollment, selectedContainer, state, startOnboarding, health, healthJobId, onRetry }: Readonly<{ enrollment: ReturnType<typeof useEnrollment>; selectedContainer: string; state: string | undefined; startOnboarding: boolean; health: HealthMutationView; healthJobId?: string; onRetry: () => void }>) {
  return <><div className="section-heading"><div><h2>Enrollment-Fortschritt</h2><p className="muted">Container <Link to={`/containers/${selectedContainer}`}>VMID {selectedContainer}</Link></p></div><span className={`status-badge ${enrollmentStatusClass(state)}`}>{enrollmentStateLabel(state)}</span></div><ol className="enrollment-steps">{steps.map(([step, label], index) => <li key={step} className={stepClass(state, step, index)}><span aria-hidden="true">{state === step ? "…" : "✓"}</span>{label}</li>)}</ol>{state && <div className="callout info"><strong>Aktueller Schritt</strong><p>{stateDescriptions[state] ?? "Der Status wird vom Controller aktualisiert."}</p></div>}{enrollment.isLoading && <p className="muted">Status wird geladen…</p>}{enrollment.error && <p className="error-state" role="alert">{enrollment.error.message}</p>}{state === "failed" && <><p className="error-state" role="alert">{enrollment.data?.failure_reason ?? "Der Agent konnte nicht registriert werden."}</p><button className="primary-button" type="button" onClick={onRetry}>Erneut versuchen</button></>}{state === "connected" && <div className="success-state">Agent verbunden. {connectedAction(startOnboarding, health, healthJobId, selectedContainer)}{health.error instanceof Error && <p className="error-state">{health.error.message}</p>}</div>}</>;
}
...
function stepClass(state: string | undefined, step: string, index: number) {
  if (state === step) return "current";
  const current = steps.findIndex(([value]) => value === (state ?? "requested"));
  return current >= index ? "complete" : "";
}
