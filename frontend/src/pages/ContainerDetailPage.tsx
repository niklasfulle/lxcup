...
import { createContainerAction, getAgentHealth, getAgentMetrics, type ContainerAction } from "../api";
...
export function ContainerDetailPage() {
  ...
  return <>
    ...
    <div className="panel-grid">
      ...
      <section className="panel"><h2>Agent</h2>{health.isLoading ? <p className="muted">Health wird geladen…</p> : health.data ? <dl className="detail-list"><dt>Verbindung</dt><dd>{health.data.healthy ? "Healthy" : "Fehler"}</dd><dt>Version</dt><dd>{health.data.info.version}</dd><dt>Plattform</dt><dd>{health.data.info.platform}</dd><dt>Hostname</dt><dd>{health.data.info.hostname}</dd></dl> : <p className="muted">Agent nicht verbunden.</p>}</section>
    </div>
    <section className="panel"><div className="section-heading"><h2>Aktionen</h2><span className="muted">Jede Aktion erzeugt einen Task</span></div><div className="action-row">{actions.map((item) => <button key={item.value} type="button" className="theme-button" disabled={action.isPending || (item.value === "start" && container.status === "running") || (item.value !== "start" && item.value !== "refresh" && container.status !== "running")} onClick={() => { if (window.confirm(`Aktion ${item.label} für ${container.name} bestätigen?`)) action.mutate(item.value); }}>{item.label}</button>)}</div>{action.data ? <p className="success-state" role="status">Task {action.data.id} angenommen · {action.data.status}</p> : null}{action.error ? <p className="error-state" role="alert">{action.error.message}</p> : null}</section>
