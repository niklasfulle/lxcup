...
export function TaskMonitor({ events, onClear, collapsed, onToggle }: TaskMonitorProps) {
  return <>
    ...
    >
      ...
      {events.length === 0 ? <p className="m-0 py-3 text-xs text-[var(--muted)]">Noch keine laufenden Aufgaben.</p> : <ul className="m-0 min-h-0 flex-1 list-none overflow-y-auto [scrollbar-color:var(--line)_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[var(--line)] [&::-webkit-scrollbar-track]:bg-transparent p-0">{events.slice(0, 8).map((item) => <li className="grid min-w-0 grid-cols-[1.5rem_minmax(0,1fr)_auto_auto] items-center gap-x-2 gap-y-1 border-b border-[var(--line)] py-2 last:border-0" key={item.id}><EventBadge item={item} /><div className="grid min-w-0 gap-1 break-words text-xs"><strong className="leading-5">{eventTitle(item)}</strong><span className="text-[var(--muted)]">{new Date(item.receivedAt).toLocaleTimeString()}</span></div><JobStatus event={item.event} /><EventLink event={item.event} /></li>)}</ul>}
    </aside>
