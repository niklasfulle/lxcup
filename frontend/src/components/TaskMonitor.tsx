...
export function TaskMonitor({ events, onClear }: TaskMonitorProps) {
  return <section className="task-monitor" aria-label="Globale Aufgaben und Ereignisse">
    <div className="section-heading"><div><h2>Aktivität</h2><span className="muted">{events.length} Ereignisse</span></div><button className="secondary-button" type="button" onClick={onClear} disabled={events.length === 0}>Aktivität leeren</button></div>
    {events.length === 0 ? <p className="muted">Noch keine laufenden Aufgaben.</p> : <ul className="activity-list">{events.slice(0, 8).map((item) => <li key={item.id}><EventBadge event={item.event} /><div><strong>{eventTitle(item.event)}</strong><span className="muted">{new Date(item.receivedAt).toLocaleTimeString()}</span></div><EventLink event={item.event} /></li>)}</ul>}
  </section>;
}
