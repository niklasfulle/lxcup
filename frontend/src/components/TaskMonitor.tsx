...
export function TaskMonitor({ events }: { events: GlobalEvent[] }) {
  return <section className="task-monitor" aria-label="Globale Aufgaben und Ereignisse">
    <div className="section-heading"><h2>Aktivität</h2><span className="muted">{events.length} Ereignisse</span></div>
...
function EventLink({ event }: { event: ApiEvent }) {
  ...
  if (event.payload.resource === "ansible_job") return <Link to="/workflows">Öffnen</Link>;
  return null;
}
