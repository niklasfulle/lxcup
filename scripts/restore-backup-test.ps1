...
missing_tables AS (
    ...
    SELECT secret_ref FROM (
        ...
        UNION SELECT api_secret_ref::text FROM proxmox_environments
        UNION SELECT ca_secret_ref::text FROM proxmox_environments WHERE ca_secret_ref IS NOT NULL
    ) refs
...
"@
    $checks = psql --dbname=$env:DATABASE_TEST_URL --tuples-only --no-align --command $integritySql
    if ($LASTEXITCODE -ne 0 -or $checks -notcontains 'missing_tables=0' -or $checks -notcontains 'bad_secret_refs=0' -or $checks -notcontains 'audit_data_invalid=0') { throw "Restored database schema, secret-reference shape, or audit data validation failed." }
    ...
    $referencedSecretIds = psql --dbname=$env:DATABASE_TEST_URL --tuples-only --no-align --command @"
...
"@
    if ($LASTEXITCODE -ne 0) { throw "Restored secret references could not be read." }
