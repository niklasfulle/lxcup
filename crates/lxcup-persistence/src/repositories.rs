...
impl NodeRepository {
    ...
    pub async fn save(&self, node: &Node) -> Result<(), RepositoryError> {
        sqlx::query(
            "INSERT INTO nodes (id, name, address, status, created_at) VALUES ($1, $2, $3, $4, $5)",
        )
    ...
    pub async fn find_by_id(&self, id: NodeId) -> Result<Option<Node>, RepositoryError> {
        let row =
            sqlx::query("SELECT id, name, address, status, created_at FROM nodes WHERE id = $1")
                .bind(id.as_uuid())
    ...
    pub async fn list(&self) -> Result<Vec<Node>, RepositoryError> {
        let rows =
            sqlx::query("SELECT id, name, address, status, created_at FROM nodes ORDER BY name")
                .fetch_all(&self.pool)
                .await?;
